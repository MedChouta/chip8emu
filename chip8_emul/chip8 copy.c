...
int main(int argc, char** argv){
    ...
    for(i = 0; i < 64*32; i++){
        ...
        SDL_FillRect(surfRect[i], NULL, SDL_MapRGB(window->format, 255, 255, 255));
...
//                printf("REG [I]: %d\n", memory.I - 512);
                for(i = 0; i < instructionSplit[3]; i++){
                    for(k = 0; k < 8; k++){
                        if ((memory.progSpace[memory.I + i] >> k) & 1){
                            for (j = 0; j < 64*32; j++){
        ...
        for(i = 0; i < 64*32; i++){
            if (r[i].w == 0){
                SDL_FillRect(surfRect[i], NULL, SDL_MapRGB(window->format, 0, 0, 0));
            }else{
                SDL_FillRect(surfRect[i], NULL, SDL_MapRGB(window->format, 255, 255, 255));
            }
            
            SDL_BlitSurface(surfRect[i], NULL, window, &r[i]);
        }
